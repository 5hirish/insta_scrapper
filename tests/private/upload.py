...
class UploadTests(ApiTestBase):
    ...
    def test_post_photo_base(self, size=(800, 800), caption='HEY', upload_id=None, location=None,
        ...
        with compat_mock.patch(
                'instascrape.endpoints.accounts.compat_urllib_request.OpenerDirector.open') as opener, \
                compat_mock.patch('instascrape.Client._read_response') as read_response, \
                compat_mock.patch('instascrape.Client.default_headers') as default_headers, \
                compat_mock.patch(
                    'instascrape.endpoints.accounts.compat_urllib_request.Request') as request, \
                compat_mock.patch('instascrape.Client._call_api') as call_api, \
                compat_mock.patch('instascrape.endpoints.upload.time.time') as time_mock, \
                compat_mock.patch('instascrape.endpoints.upload.randint') as randint_mock, \
                compat_mock.patch('instascrape.http.random.choice') as randchoice_mock:
    ...
    def test_post_video_base(self, size, duration, caption='', location=None,
                             ...
                             **kwargs):
        ts_now = time.time()
        with compat_mock.patch('instascrape.Client._call_api') as call_api, \
                compat_mock.patch(
                    'instascrape.endpoints.accounts.compat_urllib_request.OpenerDirector.open') as opener, \
                compat_mock.patch('instascrape.Client._read_response') as read_response, \
                compat_mock.patch('instascrape.Client.default_headers') as default_headers, \
    ...
    def test_post_album_mock(self):
        ts_now = time.time()
        with compat_mock.patch('instascrape.Client._call_api') as call_api, \
                compat_mock.patch('instascrape.Client.post_photo') as post_photo, \
                compat_mock.patch('instascrape.Client.post_video') as post_video, \
                compat_mock.patch('instascrape.endpoints.upload.time.time') as time_mock:
